...
long double **matrix(nrl,nrh,ncl,nch)
...
/* allocate a long double matrix with subscript range m[nrl..nrh][ncl..nch] */
{
	...
	/* allocate pointers to rows */
	m=(long double **) malloc((unsigned int)((nrow+NR_END)*sizeof(long double*)));
	if (!m) nrerror("allocation failure 1 in matrix()");
	m += NR_END;
	m -= nrl;

	/* allocate rows and set pointers to them */
	m[nrl]=(long double *) malloc((unsigned int)((nrow*ncol+NR_END)*sizeof(long double)));
	if (!m[nrl]) nrerror("allocation failure 2 in matrix()");
	m[nrl] += NR_END;
	m[nrl] -= ncl;
...
long double **dmatrix(nrl,nrh,ncl,nch)
long nch,ncl,nrh,nrl;
/* allocate a long double matrix with subscript range m[nrl..nrh][ncl..nch] */
{
	long i, nrow=nrh-nrl+1,ncol=nch-ncl+1;
...
long double **submatrix(a,oldrl,oldrh,oldcl,oldch,newrl,newcl)
long double **a;
...
long double **convert_matrix(a,nrl,nrh,ncl,nch)
long double *a;
long nch,ncl,nrh,nrl;
/* allocate a long double matrix m[nrl..nrh][ncl..nch] that points to the matrix
declared in the standard C manner as a[nrow][ncol], where nrow=nrh-nrl+1
...
long double ***f3tensor(nrl,nrh,ncl,nch,ndl,ndh)
...
/* allocate a long double 3tensor with range t[nrl..nrh][ncl..nch][ndl..ndh] */
{
	...
	/* allocate pointers to pointers to rows */
	t=(long double ***) malloc((unsigned int)((nrow+NR_END)*sizeof(long double**)));
	if (!t) nrerror("allocation failure 1 in f3tensor()");
	t += NR_END;
	t -= nrl;

	/* allocate pointers to rows and set pointers to them */
	t[nrl]=(long double **) malloc((unsigned int)((nrow*ncol+NR_END)*sizeof(long double*)));
	if (!t[nrl]) nrerror("allocation failure 2 in f3tensor()");
	t[nrl] += NR_END;
	t[nrl] -= ncl;

	/* allocate rows and set pointers to them */
	t[nrl][ncl]=(long double *) malloc((unsigned int)((nrow*ncol*ndep+NR_END)*sizeof(long double)));
	if (!t[nrl][ncl]) nrerror("allocation failure 3 in f3tensor()");
	t[nrl][ncl] += NR_END;
	t[nrl][ncl] -= ndl;
